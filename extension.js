...
Menu.actor.remove_child(Main.panel.statusArea.aggregateMenu._system._sessionSubMenu)
this._nextAction();
Menu.moveMenuItem(Main.panel.statusArea.aggregateMenu._system._settingsItem, Main.panel.statusArea.aggregateMenu._system.menu.numMenuItems-2)
Menu.moveMenuItem(Main.panel.statusArea.aggregateMenu._system._lockScreenItem, this._itemPosition(switchUser)+1)
},
...
_itemPosition: function(item) {
	let items = Menu._getMenuItems();
	for (let i=0; i < items.length; ++i) { if (items[i] == item) { return i; }
	}
},
...
Menu.box.remove_actor(Main.panel.statusArea.aggregateMenu._system._settingsItem);
Menu.box.remove_actor(Main.panel.statusArea.aggregateMenu._system._lockScreenItem);
Menu.box.insert_child_at_index(Main.panel.statusArea.aggregateMenu._system._settingsItem, Main.panel.statusArea.aggregateMenu._system.menu.numMenuItems);
Menu.box.insert_child_at_index(Main.panel.statusArea.aggregateMenu._system._lockScreenItem, Main.panel.statusArea.aggregateMenu._system.menu.numMenuItems);
