...
this._lock = this._settings.get_boolean('remove-lock-button');
if (this._lock) { Main.panel.statusArea.aggregateMenu._system._lockScreenItem.hide(); } else { Main.panel.statusArea.aggregateMenu._system._lockScreenItem.show(); }
	// Separator 1
Menu.addMenuItem(separator1);
	// Logout
this._logout = this._settings.get_boolean('remove-logout-button');
if (!this._logout) { Menu.addMenuItem(logout); }; this._systemActions.bind_property('can-logout', logout, 'visible', bindFlags);
	// Switch User
...
this._settings.connect("changed::remove-lock-button", this._takeAction.bind(this));
this._settings.connect("changed::remove-logout-button", this._takeAction.bind(this));
